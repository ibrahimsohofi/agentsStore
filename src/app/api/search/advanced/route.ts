...
function analyzeUserProfile(userOrders: any[]) {
...
// Get content-based recommendations
async function getContentBasedRecommendations(userProfile: any, query: string, category?: string, userId?: string) {
...
async function getCollaborativeRecommendations(userId: string, userProfile: any) {
  try {
...
function calculateRelevanceScore(agent: any, userProfile: any, query?: string): number {
  let score = 0
...
function calculateRelevanceScore(agent: any, query: string, userId?: string) {
  let score = 0
...
function calculatePopularityScore(agent: any) {
  return (agent.totalSales * 0.7) + (agent.rating * 20) + (agent._count.reviews * 0.5)
}

function calculateQualityScore(agent: any) {
