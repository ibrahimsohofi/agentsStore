...
async function runQualityTests(agent: any) {
  const checks = []
  let score = 0
...
async function runSecurityScan(agent: any) {
  const checks = []
  let score = 0
  ...
  if (agent.downloadUrl && agent.downloadUrl.startsWith('https://')) {
...
async function runPerformanceTest(agent: any) {
...
async function runCompatibilityCheck(agent: any) {
  const checks = []
  let score = 0
  ...
  const hasApiKeywords = (agent.description + ' ' + agent.longDescription).toLowerCase()
    .includes('api') || (agent.documentation && agent.documentation.toLowerCase().includes('api'))
  ...
  if (agent.downloadUrl && agent.downloadUrl.includes('.json')) {
...
function generateQualityRecommendations(checks: any[]) {
...
async function runComprehensiveTests(agent: any, testTypes: string[], config: any, testId: string) {
...
async function runSecurityTests(agent: any): Promise<TestResult[]> {
  const results: TestResult[] = []
...
async function runPerformanceTests(agent: any, config: any): Promise<TestResult[]> {
  const results: TestResult[] = []
...
async function runFunctionalityTests(agent: any): Promise<TestResult[]> {
  const results: TestResult[] = []
...
async function runQualityTestsNew(agent: any): Promise<TestResult[]> {
  const results: TestResult[] = []
